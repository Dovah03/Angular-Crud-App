...
*/
  ...
  */
  ...
  private changePsw(currentPassword: string, newPassword: string) {
    return this.http
      .put<User>(`${this.ApiServerUrl}/user/changepsw`, {
        currentPassword,
        newPassword,
      })
  ...
  public changePswImpl(currentPassword: string, newPassword: string) {
    return this.changePsw(currentPassword, newPassword).pipe(
      tap((res) => {
  ...
  public getCurrentUser(): Observable<any> {
    return this.http
      .get<User>(`${this.ApiServerUrl}/user/current/`)
      .pipe(catchError(this.errorHandler));
  }

  private login(username: string, password: string): Observable<any> {
    return this.http
      .post<any>(`${this.ApiServerUrl}/user/auth`, { username, password })
      .pipe(catchError(this.errorHandler));
  }
  ...
  public getClients(): Observable<any> {
    return this.http.get<Client[]>(`${this.ApiServerUrl}/user/clients`)
  }

  public getClientbyId(clientID : number):Observable<any> {
    return this.http.get<Client>(`${this.ApiServerUrl}/user/client/find/${clientID}`)
  }

  public updateClient(ClientId : number,client:Client):Observable<any> {
    return this.http.put<Client>(`${this.ApiServerUrl}/user/updateC/${ClientId}`,client);
  }
  ...
  public deactivateUser(userId: number, user: User): Observable<any> {
    return this.http.put<User>(
      `${this.ApiServerUrl}/user/deactivate/${userId}`,
      user
    );
  }
  ...
  public activateUser(userId: number, user: User): Observable<any> {
    return this.http.put<User>(
      `${this.ApiServerUrl}/user/activate/${userId}`,
      user
    );
  }
  public addDevis(ClientID: number, numoffre: number): Observable<any> {
    return this.http.post<Client>(`${this.ApiServerUrl}/user/addd/${ClientID}`, numoffre);
  }
