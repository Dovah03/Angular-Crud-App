...
export class BonLivraisonComponent implements OnInit {
  ...
  constructor(private userService:UserService,public route: ActivatedRoute, private router:Router, private fb:FormBuilder,
    private BLservice : BLService, private toastr:ToastrService, public ete: ExportExcelService, private theme:themeService,private mail:MailService,
  ...
  ngOnInit(): void {
    ...
    this.BLservice.getBlById(this.id).subscribe((res : BL) => {
      this.bl = res;
      this.showLoadingSpinner = false;
      this.dataForExcel.push(this.bl);
  ...
  onOpenModal(mode:string){
    ...
    if(mode === 'edit') {
      button.setAttribute('data-bs-target','#editBLModal');
    }
    if(mode === 'delete') {
      button.setAttribute('data-bs-target','#deleteBLModal');
    }
    if(mode === 'mail') {
  ...
  onEdit(bl:BL){
    ...
    console.log(bl.date)
    this.BLservice.updateBL(this.id,bl).subscribe(
      (response: BL)  => {
        this.showLoadingSpinner = false;
        this.toastr.success('<span class="now-ui-icons ui-1_bell-53"></span> Bon de Livraison de numero d\'offre '+response.numoffre+' modifié avec succées','Bon de Livraison modifier' ,{
          timeOut: 2000,
          enableHtml: true,
       ...
       /* console.log(response);
        setTimeout(() => {
          this.router.navigate(['/BL/'+this.id]);
          location.reload();
      }, 1000);
      */
  ...
  onDelete(){
    ...
    this.showLoadingSpinner = true ;
    this.BLservice.deleteBL(this.id).subscribe(
      (response: BL)  => {
        this.showLoadingSpinner = false ;
        this.toastr.success('<span class="now-ui-icons ui-1_bell-53"></span>Bon de Livraison Supprimer avec succées','Bon de Livraison Supprimer',{
          timeOut: 2000,
          enableHtml: true,
          toastClass: "alert alert-success alert-with-icon",
          positionClass: 'toast-' + 'top' + '-' +  'right'
      ...
      (error: HttpErrorResponse) => {
        this.showLoadingSpinner = false;
        this.toastr.error('<span class="now-ui-icons ui-1_bell-53"></span>erreur durant la suppression du Bon de Livraison , check log','erreur',{
          timeOut: 2000,
          enableHtml: true,
          toastClass: "alert alert-alert alert-with-icon",
          positionClass: 'toast-' + 'top' + '-' +  'right'
  ...
  onPDF(){
    ...
    doc.setDrawColor(40,218,163);
    doc.setFillColor(255, 0, 0)
    doc.rect(31, 204, 12, 8)

    //rec Item
    doc.setDrawColor(40,218,163);
    doc.setFillColor(255, 0, 0)
    doc.rect(31, 224, 15, 8)

    //rec Desi
    doc.setDrawColor(40,218,163);
    doc.setFillColor(255, 0, 0)
    doc.rect(31, 244, 31, 8)



      doc.addImage(ImageDate,'JPEG',20,10,120,40)
      doc.setFont('helvetica','Bold')
      doc.text( 'Details du Bon de Livraison de Numero : '+ this.bl.numoffre,53,100)
      doc.text("Issue à la date : "+this.bl.date,57,110);
      doc.setFont('Bold');
      doc.text('Référence                             :                           '+this.bl.ref,33,170)
      doc.text('Quantité                                :                          '+this.bl.qty,33,190)
      doc.text('PO                                        :                           '+this.bl.po,33,210)
      doc.text('Item                                      :                           '+this.bl.item+' DH',33,230)
      doc.text('Désignation                          :                            '+this.bl.desi,33,250)

      doc.save('ISOQUALTECH_BL.pdf');
  }

  exportToExcel() {

    let reportData = {
      title: 'Isoqualtech_Bon de Livraison',
      data: this.dataForExcel[0]
    }

    this.ete.excelBLExport(reportData);
  }
